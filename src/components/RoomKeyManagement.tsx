...
import { Key, Users, Eye } from "lucide-react";
...
export default function RoomKeyManagement() {
  ...
  const [wardenHostelType, setWardenHostelType] = useState<string | null>(null);
  const { toast } = useToast();

  const fetchWardenHostelType = async () => {
  ...
  const fetchRooms = async () => {
    ...
    // Get warden's hostel type first
    const hostelType = await fetchWardenHostelType();
    ...
    // Filter by hostel type if warden has one assigned
    if (hostelType) {
  ...
  const fetchRoomDetails = async (roomId: string) => {
    ...
    const { data: studentsData, error: studentsError } = await supabase
      .from("profiles")
      .select("id, user_id, full_name, student_id, key_number, key_issued_at")
      .eq("room_id", roomId);
  ...
  // Sort rooms within each floor numerically by room_number
    ...
    <div className="space-y-8 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {wardenHostelType && (
            <p className="text-muted-foreground capitalize">{wardenHostelType} Hostel</p>
          )}
        </div>
