...
export default function AdminDashboard() {
  useEffect(() => { document.title = "Admin Dashboard | HostelSync"; }, []);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        supabase.rpc("get_safe_complaints"),
  ...
  const handleDeleteUser = async (userId: string, profileName: string) => {
    try {
      ...
      toast({ title: "Deleted", description: `${profileName} has been permanently removed.` });
      fetchDashboardData();
    } catch (error: any) {
  ...
  const handleDeleteComplaint = async (complaintId: string) => {
    try {
      ...
      toast({ title: "Deleted", description: "Complaint has been permanently removed." });
      fetchDashboardData();
    } catch (error: any) {
  ...
  const handleRejectProfile = async (profileId: string, userIdToDelete: string, profileName: string) => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return;
      ...
      toast({ 
        title: "Rejected", 
        description: `${profileName}'s registration has been rejected and removed.` 
      });
      fetchDashboardData();
    } catch (error: any) {
      toast({ 
  ...
  const handleCreateNotice = async () => {
    try {
      ...
      const { error } = await supabase.from("notices").insert({
        ...
        hostel_type: newNotice.hostel_type === "all" ? null : (newNotice.hostel_type as "boys" | "girls"),
      ...
      setNewNotice({ title: "", content: "", is_urgent: false, target_roles: ["student"], hostel_type: "all" });
      fetchDashboardData();
    } catch (error) {
  ...
  // Filter profiles by hostel type
  const filteredProfiles = useMemo(() => {
    if (hostelFilter === "all") return profiles;
    return profiles.filter((p: any) => p.hostel_type === hostelFilter);
  }, [profiles, hostelFilter]);
      ...
      <main className="p-3 sm:p-6">
        ...
        <Tabs defaultValue="approvals" className="w-full">
          <TabsList className="w-full justify-start">
            <TabsTrigger value="approvals" className="relative">
              ...
              {stats.pendingApprovals > 0 && (
                <Badge className="ml-2 bg-red-500 text-white">{stats.pendingApprovals}</Badge>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="students" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage all student profiles and information with full edit capabilities</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <TableBody>
                      {profiles.filter(p => p.role === "student").map((profile) => (
                        <TableRow key={profile.id}>
                          ...
                          <TableCell>{profile.year_of_study ? `Year ${profile.year_of_study}` : "N/A"}</TableCell>
                          <TableCell>{profile.rooms?.room_number || "Unassigned"}</TableCell>
                          <TableCell>
                                ...
                                <DialogContent className="max-w-2xl">
                                  ...
                                  {selectedProfile && (
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                      ...
                                      <div><strong>Floor:</strong> {selectedProfile.rooms?.floor || "N/A"}</div>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      Are you sure you want to permanently delete {profile.full_name}? This will remove all their data including attendance, outing requests, and complaints. This action cannot be undone.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                                    <AlertDialogAction onClick={() => handleDeleteUser(profile.user_id, profile.full_name)}>
                                      Delete Permanently
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                                </AlertDialogContent>
          ...
          <TabsContent value="staff" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage all staff members with full edit capabilities</CardDescription>
              </CardHeader>
              <CardContent>
                {profiles.filter(p => p.role !== "student").length === 0 ? (
                    ...
                    <TableBody>
                      {profiles.filter(p => p.role !== "student").map((profile) => (
                        <TableRow key={profile.id}>
                          ...
                          <TableCell>{profile.staff_id || "N/A"}</TableCell>
                          <TableCell>{profile.departments?.name || "Unassigned"}</TableCell>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      Are you sure you want to permanently delete {profile.full_name}? This will remove their account and all related data. This action cannot be undone.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
          ...
          <TabsContent value="approvals" className="space-y-6">
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-yellow-800">
                  ...
                  <Badge className="ml-2 bg-yellow-600 text-white">
                    {profiles.filter(p => !p.is_approved && p.role !== 'student').length}
                  </Badge>
                </CardTitle>
                <CardDescription className="text-yellow-700">
                  Staff registrations require your approval to prevent unauthorized access
                </CardDescription>
              </CardHeader>
              <CardContent>
                {profiles.filter(p => !p.is_approved && p.role !== 'student').length === 0 ? (
                  <p className="text-muted-foreground text-center py-8">No pending staff approvals</p>
                ) : (
                  <div className="space-y-4">
                    {profiles.filter(p => !p.is_approved && p.role !== 'student').map((profile) => (
                      <div key={profile.id} className="border border-yellow-300 rounded-lg p-4 bg-white">
                          ...
                          <div className="space-y-2 flex-1">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {profile.departments?.name || "Unassigned"}
                              </div>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Are you sure you want to reject {profile.full_name}'s registration? This will permanently remove their account. This action cannot be undone.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => handleRejectProfile(profile.id, profile.user_id, profile.full_name)}>
                                    Reject & Delete
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
            ...
            {/* Student Approvals Section (if any unapproved students exist) */}
            {profiles.filter(p => !p.is_approved && p.role === 'student').length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                  ...
                  <CardDescription>Students awaiting approval (auto-approved by default)</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {profiles.filter(p => !p.is_approved && p.role === 'student').map((profile) => (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
                              ...
                              <div>Student ID: {profile.student_id || "N/A"}</div>
