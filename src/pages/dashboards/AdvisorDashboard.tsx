...
import { CheckCircle, XCircle, Clock, Users, MapPin, Home, AlertCircle, Search, Download } from "lucide-react";
import DashboardHeader from "@/components/DashboardHeader";
import StudentTableCell from "@/components/StudentTableCell";
import * as XLSX from "xlsx";

export default function AdvisorDashboard() {
  useEffect(() => { document.title = "Advisor Dashboard | HostelSync"; }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch ALL hometown requests at advisor stage (no department filter)
      ...
      const allPending = requests?.filter(r => r.final_status === "pending" && !r.advisor_approved_by) || [];
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((student) => {
      ...
      return matchesSearch && matchesYear && matchesDept && matchesHostel && matchesRoom;
    });
  }, [students, studentSearch, studentYearFilter, studentDeptFilter, studentHostelFilter, studentRoomFilter]);
          ...
          <TabsContent value="pending" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Hometown outing requests waiting for your review ({filteredPending.length} of {pendingRequests.length})</CardDescription>
              </CardHeader>
              <CardContent>
                {/* Pending Filters */}
                ...
                <div className="flex items-center justify-between">
                  ...
                  {approvalHistory.length > 0 && (
                    <Button size="sm" variant="outline" onClick={() => {
                      ...
                      XLSX.writeFile(wb, `advisor_approval_history_${new Date().toISOString().split("T")[0]}.xlsx`);
