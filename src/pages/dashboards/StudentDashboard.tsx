...
export default function StudentDashboard() {
  useEffect(() => { document.title = "Student Dashboard | HostelSync"; }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      // Get student's profile to get their hostel type
      ...
      const studentHostelType = profileData?.hostel_type;
      ...
      // Fetch notices for students, filtered by hostel type
      ...
      // Filter notices by student's hostel type (include notices with null hostel_type as they're for all)
      if (studentHostelType) {
        noticesQuery = noticesQuery.or(`hostel_type.eq.${studentHostelType},hostel_type.is.null`);
      }

      const { data: noticeData } = await noticesQuery;
  ...
  const handleSubmitComplaint = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Submit complaint with hostel_type
      const { error } = await supabase.from("complaints").insert({
        ...
        hostel_type: profileData?.hostel_type || null,
